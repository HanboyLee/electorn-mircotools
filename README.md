...
发布安装包：**Windows**（setup.exe）与 **macOS**（zip 内 `.app`）均由 CI 构建；元数据写入在 Windows 上验证最充分。
...
2. 进入最新版本（例如 [v1.3.3](https://github.com/HanboyLee/electorn-mircotools/releases/tag/v1.3.3)）  
3. 按系统下载：

| 系统 | 下载文件 | 怎么用 |
|------|----------|--------|
| **Windows** | `metadata-app-setup.exe` | 双击安装向导完成安装 |
| **macOS** | `metadata-app-darwin-*-*.zip`（或类似 zip 名） | 解压后得到 `.app`，拖到「应用程序」或直接打开 |
...
- 当前应用版本见 `package.json` 的 `version`（现为 **1.3.3**）  
